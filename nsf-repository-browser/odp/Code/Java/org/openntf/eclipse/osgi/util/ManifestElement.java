...
package org.openntf.eclipse.osgi.util;
...
public class ManifestElement {
	...
	public static ManifestElement[] parseHeader(String header, String value) {
		...
		parseloop: while (true) {
			...
			if (next == null)
				throw new RuntimeException("next is null"); //$NON-NLS-1$
			...
			while (c == ';') {
				...
				while (c == ':') { // may not really be a :=
					...
					if (c != '=') {
						...
						if (restOfNext == null)
							throw new RuntimeException("restOfNext is null"); //$NON-NLS-1$
			...
			while (c == '=' || c == ':') {
				...
				if (val == null)
					throw new RuntimeException("val is null"); //$NON-NLS-1$
			...
			if (c == '\0') /* end of value */
				break parseloop;
			throw new RuntimeException("reached unexpected end"); //$NON-NLS-1$
		}
	 ...
	 */
	public static Map<String, String> parseBundleManifest(InputStream manifest, Map<String, String> headers) throws IOException {
		if (headers == null)
			headers = new HashMap<>();
		...
		try {
			...
			while (true) {
				...
				if (colon == -1) /* no colon */
				{
					throw new RuntimeException("missing colon"); //$NON-NLS-1$
				}
